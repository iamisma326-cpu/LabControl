<?php
// Formulario para editar un equipo de laboratorio. Es 100% independiente.
...
// El id del equipo llega por la URL (?id=...).
if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
} else {
    $id = 0;
...
$laboratorios = [
    'lab1' => 'Laboratorio de Cómputo 1',
    'lab2' => 'Laboratorio de Cómputo 2',
    'lab5' => 'Laboratorio de Cómputo 5',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $resultado = sim_actualizar_equipo($id, [
        'codigo'      => $codigo,
        'nombre'      => $nombre,
    ...
    if ($resultado['ok']) {
        flash_set('ok', 'Equipo actualizado correctamente.');
        header('Location: gestion_laboratorios.php');
        exit;
    } else {
...
<div class="pagina-contenedor-compacto">
    ...
    <div class="disposicion-alumno">
        ...
        <main class="flex flex-col gap-8">
            ...
            <?php encabezado('Sede Central · Laboratorios', 'Editar Equipo', 'Modifica los datos del equipo y guarda los cambios.'); ?>
            ...
            <div class="grid grid-cols-1 gap-8 max-w-3xl w-full mx-auto">
                ...
                <section class="panel-cristal p-6 sm:p-8 space-y-6">
                    <div class="border-b border-neutral-900/60 pb-3 flex items-center gap-2">
                        <i data-lucide="monitor-cog" class="icono-azul"></i>
                        <h3 class="titulo-seccion">Datos del Equipo</h3>
                    </div>

<?php
// Determinar valores actuales del formulario
if (isset($_POST['codigo'])) { $form_codigo = $_POST['codigo']; } else { $form_codigo = $equipo['codigo']; }
if (isset($_POST['nombre'])) { $form_nombre = $_POST['nombre']; } else { $form_nombre = $equipo['nombre']; }
if (isset($_POST['laboratorio'])) { $form_laboratorio = $_POST['laboratorio']; } else { $form_laboratorio = $equipo['laboratorio']; }
if (isset($_POST['estado'])) { $form_estado = $_POST['estado']; } else { $form_estado = $equipo['estado']; }
?>

                    <form method="POST" action="editar_equipo.php?id=<?php echo (int)$equipo['id']; ?>" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="nombre" name="nombre" class="campo-form-alumno"
                                value="<?php echo $form_nombre; ?>" required maxlength="120">
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <select id="laboratorio" name="laboratorio" class="campo-form-alumno" required>
                                    ...
                                    <?php foreach ($laboratorios as $key => $label): ?>
                                        <option value="<?php echo $key; ?>" <?php if ($form_laboratorio === $key) { echo 'selected'; } ?>>
                                            <?php echo $label; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <select id="estado" name="estado" class="campo-form-alumno" required>
                                    <option value="">Seleccione…</option>
                                    <option value="operativo"     <?php if ($form_estado === 'operativo') { echo 'selected'; } ?>>Operativo</option>
                                    <option value="mantenimiento" <?php if ($form_estado === 'mantenimiento') { echo 'selected'; } ?>>En Mantenimiento</option>
                                    <option value="danado"        <?php if ($form_estado === 'danado') { echo 'selected'; } ?>>Dañado</option>
                                </select>
                            </div>
                        </div>
