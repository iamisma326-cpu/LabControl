<?php
// Formulario para editar un practicante que ya existe. Es 100% independiente.
...
// El id del practicante llega por la URL (?id=...).
if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
} else {
    $id = 0;
...
$practicante = sim_practicante($id);
if (!$practicante) {
    header('Location: gestion_practicantes.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $resultado = sim_actualizar_practicante($id, [
        'codigo'          => $codigo,
        'nombres'         => $nombres,
        'apellidos'       => $apellidos,
        'dni'             => $dni,
    ...
    if ($resultado['ok']) {
        flash_set('ok', 'Practicante actualizado correctamente.');
        header('Location: gestion_practicantes.php');
        exit;
    } else {
...
<div class="pagina-contenedor-compacto">
    ...
    <div class="disposicion-alumno">
        ...
        <main class="flex flex-col gap-8">
            ...
            <?php encabezado('Sede Central · Practicantes', 'Editar Practicante', 'Modifica los datos del practicante y guarda los cambios.'); ?>
            ...
            <div class="grid grid-cols-1 gap-8 max-w-3xl w-full mx-auto">
                ...
                <section class="panel-cristal p-6 sm:p-8 space-y-6">
                    <div class="border-b border-neutral-900/60 pb-3 flex items-center gap-2">
                        <i data-lucide="user-cog" class="icono-azul"></i>
                        <h3 class="titulo-seccion">Datos del Practicante</h3>
                    </div>

<?php
// Determinar valores actuales del formulario
if (isset($_POST['codigo'])) { $form_codigo = $_POST['codigo']; } else { $form_codigo = $practicante['codigo']; }
if (isset($_POST['nombres'])) { $form_nombres = $_POST['nombres']; } else { $form_nombres = $practicante['nombres']; }
if (isset($_POST['apellidos'])) { $form_apellidos = $_POST['apellidos']; } else { $form_apellidos = $practicante['apellidos']; }
if (isset($_POST['dni'])) { $form_dni = $_POST['dni']; } else { $form_dni = $practicante['dni']; }
if (isset($_POST['horario_entrada'])) { $form_hentrada = $_POST['horario_entrada']; } else { if (isset($practicante['horario_entrada'])) { $form_hentrada = substr($practicante['horario_entrada'], 0, 5); } else { $form_hentrada = '08:00'; } }
if (isset($_POST['horario_salida'])) { $form_hsalida = $_POST['horario_salida']; } else { if (isset($practicante['horario_salida'])) { $form_hsalida = substr($practicante['horario_salida'], 0, 5); } else { $form_hsalida = '17:00'; } }
?>

                    <form method="POST" action="editar_practicante.php?id=<?php echo (int)$practicante['id']; ?>" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="codigo" name="codigo" class="campo-form-alumno"
                                value="<?php echo $form_codigo; ?>" required maxlength="20">
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="text" id="nombres" name="nombres" class="campo-form-alumno"
                                    value="<?php echo $form_nombres; ?>" required maxlength="100">
                            </div>
                            <div class="space-y-2">
                                ...
                                <input type="text" id="apellidos" name="apellidos" class="campo-form-alumno"
                                    value="<?php echo $form_apellidos; ?>" required maxlength="100">
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="dni" name="dni" class="campo-form-alumno font-mono"
                                value="<?php echo $form_dni; ?>" required pattern="\d{8,15}" maxlength="15">
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="time" id="hentrada" name="horario_entrada" class="campo-form-alumno"
                                    value="<?php echo $form_hentrada; ?>" required>
                            </div>
                            <div class="space-y-2">
                                ...
                                <input type="time" id="hsalida" name="horario_salida" class="campo-form-alumno"
                                    value="<?php echo $form_hsalida; ?>" required>
                            </div>
                        </div>
